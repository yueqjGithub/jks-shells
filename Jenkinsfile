            ...
            steps {
                script {
                    ...
                    if (env.CD_CUSTOM_PARAM == null | env.CD_CUSTOM_PARAM == ''){
                        ...
                        for (cRow in arr) {
                            def cName = sh("bash ./custom_string_parse.sh '${cRow}' 字段名").trim()
                            def cDesc = sh("bash ./custom_string_parse.sh '${cRow}' 描述").trim()                            
                            def cType = sh("bash ./custom_string_parse.sh '${cRow}' 表单类型").trim()
                            def cDefaultValue = sh("bash ./custom_string_parse.sh '${cRow}' 默认值").trim() 
                            def cOption = sh("bash ./custom_string_parse.sh '${cRow}' 选项").trim()
