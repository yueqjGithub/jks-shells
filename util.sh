...
function avalon_web_cd_pull_repo() {
    ...
    if [[ ${repoType} == "git" ]]; then
        ...
        mv .[^.]* ${WORKSPACE}/build/${buildSubPath}/
...
function avalon_web_cd_build_app() {
    ...
    for app in ${apps[@]}; do
        ...
        if [ ${appType} == 'front' ]; then
            ...
            echo "执行构建"
            if [ ${CD_WEB_MODE} != null ] && [ ${CD_WEB_MODE} != 'normal' ]; then
                npm run release:${CD_WEB_MODE} || exit 1
            else
...
cat ${deployDir}/update_tmp/appNameAndType.txt | while read appStr
do
    if [[ "\${appStr}" == "" ]]; then
      continue
    fi
    ...
    if [[ -f "${deployDir}/update_tmp/\${appName}.zip" ]]; then
      cd ${deployDir}/update_tmp/ || exit 1
      unzip -o \${appName}.zip || exit 1
      echo "已解压应用${deployDir}/update_tmp/\${appName}.zip"
      cd ${deployDir} || exit 1
    fi
